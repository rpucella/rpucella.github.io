...
# recursive closures
# 
# more effective CPS translation
# compiling environment lookup
# first stab at a register machine for compilation
#
...
def shell_cps ():
    ...
    # A version of the shell that performs a CPS transformation before
    # evaluating code
...
def shell_compenv ():
    ...
    # A version of the shell that performs a CPS transformation and also
    # compiles identifier lookup (transforming EId into EIdIndex) before
    # evaluating code
    
    print "Lecture 8 - REF Compiled Language (defun, define, CPS, compiled env)"
    print "#quit to quit, #abs to see abstract representation"
...
############################################################
# REGISTER MACHINE FOR COMPILATION



# the instructions (coded as integers)
...
def encode (code):
    ...
    # transform an array of code where instructions are written
    # as strings into one where instructions have their actual
    # coded value (for execution)

    def transform (c):
        return CODE[c] if type(c) is str else c

    return [ transform(c) for c in code ] 
...
def execute (code,start_pc,start_env):
    ...
    # execute the register machine code in array 'code'
    # starting at 'start_pc' with initial environment
    # 'start_env'
...
def test (n):
    ...
    # construct a simple sequence of instructions to sum all
