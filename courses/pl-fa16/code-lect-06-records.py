...
class ERecord (Exp):
    ...
    def __str__ (self):
        return "ERecord([{}])".format(",".join([ "({},{})".format(id,str(exp)) for (id,exp) in self._bindings ]))
...
class EWith (Exp):
    ...
    def __str__ (self):
        return "EWith({},{})".format(str(self._record),str(self._exp))
...
class VRecord (Value):
    ...
    def __str__ (self):
        return "<record {}>".format(",".join([ id+":"+(str(v)) for (id,v) in self.bindings]))
...
def parse (input):
    ...
    pRECORD = "(" + Keyword("record") + pBINDINGS + ")"
    pRECORD.setParseAction(lambda result: ERecord(result[2]))
    ...
    pEXPR << (pINTEGER | pBOOLEAN | pIDENTIFIER | pIF | pLET | pFUN | pFUNrec| pRECORD | pWITH | pDO | pWHILE | pCALL)
...
def shell ():
    ...
    print "Lecture 6 - Rec Language (defun, define)"
