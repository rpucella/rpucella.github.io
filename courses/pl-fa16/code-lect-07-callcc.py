...
class ECallCC (Exp):
    def __init__ (self,exp):
        self._exp = exp
        self.expForm = "ECallCC"

    def __str__ (self):
        return "ECallCC({})".format(str(self._exp))

    def real_eval (self,env):
        raise Exception("Runtime error: directly executing an ECallCC")
    ...
    def cps (self,k):
        var = " _k_{}_".format(fresh())
        return self._exp.cps(EFunction([var],ECall(EId(var),[EFunction(["v","_"],ECall(k,[EId("v")])),k])))
...
def shell_cps ():
    ...
    print "Lecture 7 - REF Language (defun, define, tail calls, CPS, call/cc)"
    print "#quit to quit, #abs to see abstract representation"
    env = initial_env_cps()
        
    while True:
        inp = raw_input("ref/callcc> ")
