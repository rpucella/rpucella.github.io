...
class VObject (Value):
    ...
    def env (self,obj):
        env = self._fields + [ (id,v.apply([obj])) for (id,v) in self._methods ]
        if self._super:
            # if there's a super object, add its environment in _after_ the current one
...
def parse (input):
    ...
    pCLASSsuper = "(" + Keyword("class") + "(" + pNAMES + ")" + Literal("(") + pEXPR + pEXPRS + ")" + Literal("(") + pBINDINGS + ")" + Literal("(") + pBINDINGS + Literal(")") + ")"
    pCLASSsuper.setParseAction(lambda result: EClass(result[3],result[10],result[13],result[6],result[7]))


    pNEW = "(" + Keyword("new") + pEXPR + pEXPRS +")"
    pNEW.setParseAction(lambda result: ENew(result[2],result[3]))

    pWITHOBJ = "(" + Keyword("with") + pEXPR + pEXPR +")"
    pWITHOBJ.setParseAction(lambda result: EWithObj(result[2],result[3]))
    ...
    pEXPR << (pINTEGER | pBOOLEAN | pIDENTIFIER | pIF | pLET | pFUN | pFUNrec| pCLASS | pCLASSsuper | pNEW | pWITHOBJ | pDO | pWHILE | pCALL)
