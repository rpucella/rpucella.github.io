...
module S : sig
  ...
  let rec smash prim renv =
    ...
    | P_First (s1) -> P_First (smash_comp s1 renv)
  ...
  let rec eval comp primitive =
    ...
    | P_First (a) -> let a' = eval_comp a in eval comp (P_Cst (hd a'))
  ...
  let network inputs output lst =
    ...
    let rec convert s renv =
      ...
      | First (w1) -> NC (P_First (NC (P_Wire w1, renv)),renv)
  ...
  let first (s:string):'a syntax = First (s)
...
let first : string -> 'a syntax = S.first
