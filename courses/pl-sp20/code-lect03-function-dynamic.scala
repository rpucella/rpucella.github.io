...
abstract class Value {
  ...
  def apply2 (v1 : Value, v2 : Value) : Value = {
    throw new Exception("Value " + this.toString() + " is not of type CLOSURE-2")
  }
}
...
class VClosure2 (val param1 : String, val param2 : String, val body : Exp, val env : Env) extends Value {

  override def toString () : String =
    "VClosure2(" + param1 + ", " + param2 + ", " + body.toString() +
      env.toString() + ")"

  override def apply2 (arg1 : Value, arg2 : Value) : Value = {
...
class ELetFun2 (val fn : String, val param1 : String, val param2 : String, val fbody : Exp, val ebody : Exp) extends Exp {

  override def toString () : String =
    "ELetFun1(" + fn + ", " + param1 + ", " + param2 + ", " +
      fbody.toString() + ", " + ebody.toString() + ")"

  def eval (env : Env) : Value = {
    val vfn = new VClosure2(param1, param2, fbody, env)
    val new_env = env.push(fn, vfn)
    return ebody.eval(new_env)
  }
}

// (new ELetFun1("square", "y", new ETimes(new EId("y"), new EId("y")), new EApply1("square", new EInteger(9)))).eval(new Env(List()))

// (new ELetFun2("apply-twice", "f", "x", new EApply1("f", new EApply1("f", new EId("x"))), new ELetFun1("square", "y", new ETimes(new EId("y"), new EId("y")), new EApply2("apply-twice", new EId("square"), new EInteger(9))))).eval(new Env(List ()))
...
scala> (new ELet("x", new EInteger(10), new ELetFun1("f", "y", new EId("x"), new EApply1("f", new EInteger(0))))).eval(new Env(List()))
res23: Value = VInteger(10)

scala> (new ELet("x", new EInteger(10), new ELetFun1("f", "y", new EId("x"), new ELet("x", new EInteger(20), new EApply1("f", new EInteger(0)))))).eval(new Env(List()))
res24: Value = VInteger(10)
