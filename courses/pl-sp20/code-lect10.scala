...
abstract class Exp {
  ...
  //COMPILER: compile an expression into a sequence of instructions
  //          that start at address `addr`
...
class EApply (val fn: Exp, val args: List[Exp]) extends Exp {
  ...
  def compile (addr:Int) : Vector[Opcode] = {
     ...
     code = code ++ cfn
     caddr += cfn.length
     return Vector(Op_PUSH_ENV(),
                   Op_PUSH_ADDR(caddr + 2)) ++
            code ++
            Vector(Op_OPEN(),
                   Op_JUMP(),
                   Op_SWAP(),
...
class EFunction (val recName: String, val params: List[String], val body: Exp) extends Exp {
  ...
  def compile (addr:Int) : Vector[Opcode] = {
     ...
     cfun = Vector( Op_PUSH_ADDR(addr+4),
                    Op_CLOSURE(),
                    Op_ADD_ENV(recName)) ++
            params.map((n) => Op_ADD_ENV(n)).toVector ++
...
class ELet (val bindings: List[(String, Exp)], val body: Exp) extends Exp {
  ...
  def compile (addr:Int) : Vector[Opcode] = {
     ...
     result = result ++ bindings.map((p) => Op_ADD_ENV(p._1)).toVector
...
//COMPILER: The third entry is the CodeValue used by the virtual machine
//          for the initial environment
  ("true", new VBoolean(true), CVInteger(1)),
  ("false", new VBoolean(false), CVInteger(0)),
  ("*", new VPrimitive(operTimes), CVClosure(3, new Env(List()))),
  ("+", new VPrimitive(operPlus), CVClosure(6, new Env(List()))),
  ("=", new VPrimitive(operEqual), CVClosure(9, new Env(List()))),
  ("<", new VPrimitive(operLess), CVClosure(12, new Env(List()))),
...
//COMPILER: the compiler does not support vectors or reference cells,
//          so this is commented out for now
//          Exercise: add vectors & reference cells to the virtual machine
...
//COMPILER: the initial environment for the virtual machine
val initCodeEnv: Env[CodeValue] = new Env(initBindings.map((p) => (p._1, p._3)))

//COMPILER: the initial code with the pre-compiled primitive operations code
...
def shell (): Unit = {
  ...
  //COMPILER: initializing the virtual machine with the initial code
  val code = new Code()
  ...
  while (true) {
    ...
    try { 
      ...
      if (input.trim() == "#quit") { 
        ...
        val e = parse(input)
        //COMPILER: address where the compiled code will go
        val caddr = code.length()
        //COMPILER: compile the code
        val c = e.compile(caddr) ++ Vector(Op_STOP())
        //COMPILER: add the code to the stored sequence of instructions
        code.append(c)
        //COMPILER: execute the just compiled code
...
def evalCompileFile (file: String): CodeValue = {
  var env = initEnv
  var cenv = initCodeEnv
  val code = new Code()
  code.append(initCode)
...
class Code {
   ...
   def execute (i:Int,env:Env[CodeValue]) : CodeValue = {
     val stack: Stack[StackEntry] = new Stack()
      ...
      try { 
      while (true) {
        //COMPILER: uncomment to see running commentary of the
        //          execution
      /*
        println("------------------------------")
      ...
      */
        content(PC) match {
           ...
           case Op_ADD_ENV(n) =>
               ...
               PC = PC + 1
           case Op_LOOKUP(n) =>
               stack.push(SCodeValue(ENV.lookup(n)))
               PC = PC + 1
...
//COMPILER: representation of values in the virtual machines
...
//COMPILER: representation of an entry in the virtual machine stack
...
//COMPILER: instructions of the virtual machine
...
case class Op_ADD_ENV (n: String) extends Opcode {
  override def toString (): String = "ADD-ENV(" + n + ")"
}
case class Op_LOOKUP (n: String) extends Opcode {
  override def toString (): String = "LOOKUP(" + n + ")"
}
...
//COMPILER: primitive operations for the virtual machine
