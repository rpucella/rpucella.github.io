...
class SExpParser extends RegexParsers { 
  ...
  def expr_loop: Parser[Exp] =
    LP ~ LOOP ~ ID ~ LP ~ bindings ~ RP ~ expr ~ RP ^^
      { case _ ~ _ ~ n ~ _ ~ bindings ~ _ ~ body ~ _ => new ELoop(n, bindings, body) }
...
class NextIteration(val name: String, val values: List[Value]) extends Exception(name)
...
class ELoop (val name: String, val init: List[(String, Exp)], val body: Exp) extends Exp {

  override def toString(): String = {
    val bs1 = init.map((p) => "(" + p._1 + ", " + p._2.toString() + ")")
    val bs2 = bs1.addString(new StringBuilder(), "", ", ", "").toString()
    "ELoop[" + name + ", " + bs2 + ", " + body.toString() + "]"
  }
  ...
  def eval (env: Env): Value = {
    ...
    var values = init.map((p) => p._2.eval(env))
    while (true) {
      ...
      try {
        ...
        case e : NextIteration => if (e.name == name) values = e.values else throw e
      }
    }
