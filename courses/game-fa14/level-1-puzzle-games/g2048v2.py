...
# 
# v2: use rotations for move_left, move_right, move_up, move_down
#     (note that we could use transpositions and reflections too, but iterating
#      a single kind of rotation is sufficient to get all board orientations)
...
def rotate (board,n):
    for _ in range(n):
        board = [ board[index(GRID_SIZE-j+1,i)]
                    for j in range(1,GRID_SIZE+1) 
                  for i in range(1,GRID_SIZE+1) ]
    return board


MOVE_MAP = {       # This map tells you for every move how many rotations
    'u': (1,3),    # to apply to the board before doing a move_left and
    'd': (3,1),    # then how many rotations to get the board back in its
    'l': (0,0),    # original orientation. (Note that a left move requires
    'r': (2,2)     # no rotations)
}
...
def update_board (board,move):
    board,changed = move_left(rotate(board,move[0]))
    if changed:
        board = add_random_free_cell(board)
    return rotate(board,move[1])
