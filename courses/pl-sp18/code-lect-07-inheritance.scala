...
class VObject (val supr: Value, val fields: List[(String,Value)], val methods:List[(String,Value)]) extends Value {

  /* INHERITANCE -- new field supr holding an object of the inherited class
                    look up in supr if field or method not in current object */
  ...
  override def lookupMethod (s:String) : Value = {
     ...
     if (supr.isNone) { 
       ...
       return supr.lookupMethod(s)
     }
  }
}

class EObject (val supr: Exp, val fields: List[(String,Exp)], val methods:List[(String,Exp)]) extends Exp {

  /* INHERITANCE -- new argument supr with an expression used to create an object
                    of the inherited class to store in VObject */

   override def toString () : String =
     "EObject(" + supr + "," + fields + "," + methods + ")"

   def eval (env : Env) : Value = {
     val supr_val = supr.eval(env)
     ...
     return new VObject(supr_val,fields_val,meths_val)
   }
}
...
class SExpParser extends RegexParsers { 
   ...
   def mkClass (name:String, params:List[String], supr: Exp, fields:List[(String,Exp)], meths:List[(String,Exp)]) : Exp = { 
     return new ERecFunction(name,params,new EObject(supr,fields,meths))
   }
   ...
   /* INHERITANCE -- alternative syntax for class that takes an extra
                     argument after the list of constructor parameters
                  that extra argument is an expression used to build an
		  object of the class you want to inherit from */
		     
   def expr_class_extends : Parser[Exp] =
     LP ~ CLASS ~ ID ~ LP ~ rep(ID) ~ RP ~ expr ~ LP ~ FIELDS ~ rep(binding) ~ RP ~ LP ~ METHODS ~ rep(meth_binding) ~ RP ~ RP ^^
        { case _ ~ _ ~ rec ~ _ ~ params ~ _ ~ supr ~ _ ~ _ ~ fields ~ _ ~ _ ~ _ ~ meths ~ _ ~ _ => mkClass(rec,params,supr,fields,meths) }

   def expr_class : Parser[Exp] =
     LP ~ CLASS ~ ID ~ LP ~ rep(ID) ~ RP ~ LP ~ FIELDS ~ rep(binding) ~ RP ~ LP ~ METHODS ~ rep(meth_binding) ~ RP ~ RP ^^
        { case _ ~ _ ~ rec ~ _ ~ params ~ _ ~ _ ~ _ ~ fields ~ _ ~ _ ~ _ ~ meths ~ _ ~ _ => mkClass(rec,params,new ELiteral(VNone),fields,meths) }
   ...
   def expr : Parser[Exp] =
      ( atomic | expr_if | expr_vec | expr_fun | expr_funr | expr_let | expr_do | expr_while | expr_field | expr_meth | expr_class_extends | expr_class | expr_app) ^^
           { e => e }
