...
Blog posts are [handled in the same way](/blog/posts/2023/static-web-site-generation-3/). A preliminary transformation takes all posts stored in subdirectories of `/blog/_src/` (where each such subdirectory contains a `_post.md` file with the content of the post in markdown format), and copy all those subdirectories into `/blog/posts/` subdirectories and creating in each a `_src/` subdirectory containing an `index.md` file filled with the content of the original `_post.md` file. Once that transformation is done, the normal markdown ⟶ content ⟶ html cascade kicks in, turning the markdown post into an HTML page containing the text of the post, embedded in the website.

Two things are key for my support of _Previous_ and _Next_ buttons on each post:this framework:
